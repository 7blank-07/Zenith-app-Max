...
function backendCandidates() {
  ...
  const values = [configured, ...LOCAL_API_BASE_URLS]
...
function isPhantomRecord(normalized) {
  // Relaxed check: Real cards MUST have a cardBackground or playerImage, and a positive OVR.
  if (!(normalized.cardBackground || normalized.playerImage) || !normalized.ovr || normalized.ovr <= 0) {
    return true;
  }
...
export async function GET(request) {
  ...
  return NextResponse.json({ 
    players: [],
    pagination: { total: 0, limit: fallbackLimit, offset: fallbackOffset, has_more: false },
