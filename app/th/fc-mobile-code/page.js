...
const routeConfig = getRedeemRouteConfigByKey(REDEEM_ROUTE_KEY.THAILAND);

export async function generateMetadata() {
  return buildRedeemRouteMetadata(routeConfig);
}

export default async function RedeemCodesThailandPage({ searchParams = {} }) {
  ...
  const pageData = await getRedeemRoutePageData(REDEEM_ROUTE_KEY.THAILAND, {
    search,
    section
  });
    ...
    <SiteChrome activeView="home">
      <main className="main-content">
        {schemas.map((schema, index) => (
          <script
            key={`redeem-thailand-schema-${index}`}
