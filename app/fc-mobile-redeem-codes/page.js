...
const routeConfig = getRedeemRouteConfigByKey(REDEEM_ROUTE_KEY.GLOBAL);

export async function generateMetadata() {
  return buildRedeemRouteMetadata(routeConfig);
}

export default async function RedeemCodesGlobalPage({ searchParams = {} }) {
  ...
  const pageData = await getRedeemRoutePageData(REDEEM_ROUTE_KEY.GLOBAL, {
    search,
    section
  });
    ...
    <SiteChrome activeView="home">
      <main className="main-content">
        {schemas.map((schema, index) => (
          <script
            key={`redeem-global-schema-${index}`}
