...
const routeConfig = getRedeemRouteConfigByKey(REDEEM_ROUTE_KEY.INDONESIA);

export async function generateMetadata() {
  return buildRedeemRouteMetadata(routeConfig);
}

export default async function RedeemCodesIndonesiaPage({ searchParams = {} }) {
  ...
  const pageData = await getRedeemRoutePageData(REDEEM_ROUTE_KEY.INDONESIA, {
    search,
    section
  });
    ...
    <SiteChrome activeView="home">
      <main className="main-content">
        {schemas.map((schema, index) => (
          <script
            key={`redeem-indonesia-schema-${index}`}
