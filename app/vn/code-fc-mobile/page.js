...
const routeConfig = getRedeemRouteConfigByKey(REDEEM_ROUTE_KEY.VIETNAM);

export async function generateMetadata() {
  return buildRedeemRouteMetadata(routeConfig);
}

export default async function RedeemCodesVietnamPage({ searchParams = {} }) {
  ...
  const pageData = await getRedeemRoutePageData(REDEEM_ROUTE_KEY.VIETNAM, {
    search,
    section
  });
    ...
    <SiteChrome activeView="home">
      <main className="main-content">
        {schemas.map((schema, index) => (
          <script
            key={`redeem-vietnam-schema-${index}`}
