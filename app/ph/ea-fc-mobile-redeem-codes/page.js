...
const routeConfig = getRedeemRouteConfigByKey(REDEEM_ROUTE_KEY.PHILIPPINES);

export async function generateMetadata() {
  return buildRedeemRouteMetadata(routeConfig);
}

export default async function RedeemCodesPhilippinesPage({ searchParams = {} }) {
  ...
  const pageData = await getRedeemRoutePageData(REDEEM_ROUTE_KEY.PHILIPPINES, {
    search,
    section
  });
    ...
    <SiteChrome activeView="home">
      <main className="main-content">
        {schemas.map((schema, index) => (
          <script
            key={`redeem-philippines-schema-${index}`}
